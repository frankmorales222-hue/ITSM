...
type Action={id:number;ticket_id?:number;ticket_number?:string;ticket_subject?:string;action_type:string;target:string;status:string;result_summary?:string;created_at:string}
...
export function EndpointActions({ticket,user}:Props){
 ...
 useEffect(()=>{load()},[load])
 useEffect(()=>{if(!isRequester||!actions.some(item=>item.status==='Pending approval'))return;const refresh=()=>{if(!document.hidden)load()};const timer=setInterval(refresh,5000);addEventListener('focus',refresh);document.addEventListener('visibilitychange',refresh);return()=>{clearInterval(timer);removeEventListener('focus',refresh);document.removeEventListener('visibilitychange',refresh)}},[isRequester,actions.some(item=>item.status==='Pending approval'),load])
